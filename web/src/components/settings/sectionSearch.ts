    ...
    rows: [
      ...
      { anchorId: 'attentionDesktopAlerts', terms: ['Desktop alerts'] },
      {
        ...
        terms: [
          ...
          // Synonyms for the surviving desktop channel. WARDEN-1274 retired the
          // attention alert (and with it `mute` / `bell` / `attention alert`);
          // what this switch still raises is a watch-pattern or token-budget
          // ping, so `watch pattern` is the term its copy actually ships.
          'unfocused',
          'permission',
          'watch pattern',
        ],
      ...
      // WARDEN-1360: 'Erroring' / 'Waiting on you' / 'Blocked' are gone with
      // the unsubstantiated buckets they gated — an anchor to a removed toggle
      // resolves to nothing, so only the observable states remain.
      ...
      // webhook 'Attention' routing switch are gone with the attention alert
      // channel they routed, so they get no anchor entries here — an anchor
      // that resolves to a control the user cannot find is worse than none.
      ...
      // `Which alerts to push` heads the per-alert routing block; its first
      // surviving control is the token-budget switch, so it borrows that
      // anchor. `test alert` is the "Send test alert" BUTTON, which carries no
      // id of its own — it stays an anchor-less synonym rather than pointing
      // the highlight at an unrelated row.
      { anchorId: 'webhookAlertBudget', terms: ['Which alerts to push', 'Token budget'] },
      { terms: ['test alert'] },
    ],
  },
