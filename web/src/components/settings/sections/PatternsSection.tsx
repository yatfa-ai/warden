...
export function PatternsSection({ config, setConfig, hidden }: PatternsSectionProps) {
  ...
  // synthesized. WARDEN-1359: the "Name (copy)" suffix loop now comes from the
  // shared nonCollidingCopyName helper (lib/copyName) — the same tested loop the
  // snippet/preset duplicate handlers use — with behavior unchanged: the copy
  // gets a non-colliding "Name (copy)" / "Name (copy 2)" … suffix, truncated so
  // a source name already at the WATCH_PATTERN_NAME_MAX cap doesn't overflow
  // once suffixed. validatePatternName only ever answers 'duplicate' on these
  // candidates (a suffixed copy is never empty/too-long), so the loop always
  // terminates on a free slot.
  const duplicatePattern = (id: string) => {
    ...
    const copyName = nonCollidingCopyName(
      src.name,
      (candidate) => validatePatternName(candidate, config.watchPatterns) === 'duplicate',
      WATCH_PATTERN_NAME_MAX,
    );
    setConfig({
      ...config,
      watchPatterns: [...config.watchPatterns,
        { id: genPatternId(), name: copyName, expression: src.expression, mode: src.mode, enabled: src.enabled }],
    });
  };
