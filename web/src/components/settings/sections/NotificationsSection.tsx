    ...
    <SettingsSection title="Notifications" className={hidden ? 'hidden' : undefined}>
      ...
      <div className="flex flex-col gap-3 rounded-md border border-border/60 p-3 mt-2">
        ...
        {/* Per-state toggle (WARDEN-344): which pane states raise the
            ...
            "stuck" without losing "finished". Same client-side persistence as
            the master toggle above. WARDEN-1274: these are purely DISPLAY
            filters on the passive badge/rundown — the desktop alert they also
            gated is retired, so nothing here interrupts the human.
            WARDEN-1360: only the states the passive readout can substantiate
            remain — erroring/waiting/blocked were substring guesses over pane
            text (a passing suite's "0 errors" line classified as erroring
            while a real crash read idle), so their buckets and their knobs
            are gone. */}
        <div className="flex flex-col gap-2">
