 ...
 *
 * WARDEN-1416: every category in the registry is `collecting` today. `names`
 * USED to be the lone `decorating` entry and was the design's named dead
 * switch; it now produces the bounded `workspace-names` aggregate. The role
 * stays in the vocabulary because it is load-bearing machinery
 * (`collectsEvents`, the transparency catalog's `inert` flag, the Settings
 * surface), not dead code — a future decorate-only category would use it.
 */
 ...
 * COLLECTING category is on. This is the pipeline's "off = nothing" gate, and it
 * opens automatically for a future collecting category without the gate's code
 * changing.
 *
 * WARDEN-1416: every registry category is `collecting` today (names joined when
 * it got its own `workspace-names` producer), so this is now equivalent to
 * "any category on". The role machinery stays: a future decorate-only category
 * would be inert alone under exactly this gate.
 */
