...
// frequency remote op (it fires on every observer poll — readChats,
// alertChangedAgents — PLUS the 2s monitor tick),
...
// polls on readChats/alertChangedAgents.
...
function printCaptureProjection(hosts, ticks) {
  ...
  console.log('observer poll (readChats/');
...
async function liveScriptDeliveryBenchmark(host, ticks) {
  ...
  console.log('per-op ssh spawns. (The remote web-pane attach joined them in WARDEN-1295:');
  console.log('under the toggle its PTY is host-side and rides the channel too — the CLI');
  console.log('attachInteractiveTmux dev tool and the toggle-off default excepted.)');
  console.log();
}
...
async function main() {
  ...
  console.log('warden companion-transport benchmark  (WARDEN-272 + WARDEN-276 + WARDEN-382 + WARDEN-386 + WARDEN-409 + WARDEN-888 + WARDEN-1261 + WARDEN-1284 + WARDEN-1295 / roadmap WARDEN-270)\n');
