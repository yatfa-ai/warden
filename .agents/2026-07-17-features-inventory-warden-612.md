...
- **StatusDot (connection/exit)** — Entry: header → leftmost dot + label. Does: connected=green solid, connecting=yellow pulse, session_dead=red square "Session ended", host_unreachable=red square "Unresponsive", error=red square; dimmed (agent exited)=gray "Exited". Notes: `keep` on-exit behavior leaves a misleading "Connecting" yellow dot (PaneTile.tsx:796-799). ☐
...
- **`keep` on-exit leaves a misleading "Connecting" dot** — known UX wrinkle called out in code.
...
- **Dashboard Refresh Interval (ms)** — Entry: number input `min=10000 max=120000 step=5000`. Does: `setConfig({pollIntervalMs})`. Notes: **internally inconsistent** — state default `1500`, no clamping, so it can show/save 1500 below its own min; dashboard enforces a 10s floor and silently uses 60s for 1500. Displayed value ≠ cadence you get. ☐
- **Tmux Session Name / Connect Timeout (s)** — Entry: text / number (`min=1 max=60`, default 10). Server config. Notes: no client clamping; relies on backend. ☐
...
- **Dashboard Refresh Interval inconsistent** — default 1500, `min=10000`, no clamp; can show/save 1500 while dashboard uses 60s.
- **Numeric inputs rely on backend clamping** — connectTimeout/session-timeout/token-budget/thresholds use bare `parseInt(... || default)`; HTML min/max are advisory.
...
2. **Dashboard Refresh Interval is inconsistent.** Settings shows/saves a value (default 1500) below its own stated `min=10000`; the dashboard silently substitutes 60s and the CLI reads the raw value. The displayed number is not the cadence you get.
3. **`keep` on-exit leaves a misleading "Connecting" dot.** A pane whose agent has exited (on-exit pref `keep`) continues to show the yellow pulsing "Connecting" StatusDot instead of reflecting the exit (PaneTile.tsx:796-799).
...
17. **Numeric inputs rely on backend clamping** — connectTimeout / session-timeout / token-budget / health thresholds accept out-of-range typing (HTML min/max advisory only).
...
8. **Settings (Area 7).** Walk all 13 sections. Toggle theme/density/font live. Check the Refresh Interval field shows/saves 1500 but dashboard uses 60s (#2). Notifications toggles. Webhook/Telemetry "Test" buttons (note webhook needs Save first). Reset section visibility (#16).
